...
EspMeshMesh::Impl::Impl() {
  mWifiDrv = wifiDrvFactory();
}

void EspMeshMesh::Impl::setup(SetupConfig *config) {
  mTeardownDeadline = 0;

  if(config->uart.baudRate > 0) {
    mUart = uartFactory();
    mUart->setBaudRate(config->uart.baudRate);
    mUart->setTxBuffer(config->uart.txBuffer);
    mUart->setRxBuffer(config->uart.rxBuffer);
    mUart->setIncomingFrameCallback(std::bind(&Impl::handleUartFrame, this, _1, _2));
    mUart->setup();
  }
  ...
  auto handler = std::bind(&Impl::handleFrame, this, _1, _2, _3, _4);
  ...
  mConnectedPath = new ConnectedPath(packetbuf);
...
void EspMeshMesh::Impl::setAesPassword(std::string password) {
...
void EspMeshMesh::Impl::dumpConfig() {
  ...
  if(mUart) mUart->dump_config();
  if(mWifiDrv) mWifiDrv->dump_config();
#ifdef IDF_VER
...
void EspMeshMesh::Impl::shutdown() {
  LIB_LOGI(TAG, "Shutting down meshmesh...");
  mConnectedPath->shutdown();
  mTeardownPhase = 1;
}

bool EspMeshMesh::Impl::teardown() {
  bool teardown = false;
...
const std::string EspMeshMesh::Impl::libVersion() const { return ESPMESHMESH_VERSION; }

void EspMeshMesh::Impl::uartSendData(const uint8_t *buff, uint16_t len) {
  if(mUart) mUart->sendFramedData(buff, len);
  else LIB_LOGE(TAG, "uartSendData no uart");
}

void EspMeshMesh::Impl::broadcastSendData(const uint8_t *buff, uint16_t len) {
  if (broadcast)
    broadcast->send(buff, len);
}

void EspMeshMesh::Impl::broadcastSendData(const uint8_t *buff, uint16_t len, uint16_t port) {
  if (broadcast2)
    broadcast2->send(buff, len, port, nullptr);
}

bool EspMeshMesh::Impl::broadCastBindPort(uint16_t port, ProtocolReceiveHandler handler) {
  if (broadcast2)
    return broadcast2->bindPort(port, handler);
  return false;
}

void EspMeshMesh::Impl::unicastSendData(const uint8_t *buff, uint16_t len, uint32_t addr, uint16_t port) {
  if (unicast)
    unicast->send(buff, len, addr, port, nullptr);
}

void EspMeshMesh::Impl::multipathSendData(const uint8_t *buff, uint16_t len, uint32_t addr, uint8_t pathlen, uint32_t *path) {
  if (!multipath)
    return;
...
void EspMeshMesh::Impl::setLockdownMode(bool active) {
  if(mWifiDrv) mWifiDrv->setLockdownMode(active);
}

void EspMeshMesh::Impl::commandReply(const uint8_t *buff, uint16_t len) {
...
void EspMeshMesh::Impl::handleUartFrame(const uint8_t *data, uint16_t length) {
  handleFrame(data, length, MeshAddress(MeshAddress::SRC_SERIAL, 0, 0), 0);
}

void EspMeshMesh::Impl::handleFrame(const uint8_t *data, uint16_t len, const MeshAddress &from, int16_t rssi) {
  ...
  switch (data[0]) {
    ...
    case CMD_DISCOVERY_REQ:
      if (len > 1) {
        err = mDiscovery.handle_frame(data + 1, len - 1, from, rssi);
      }
      break;
    ...
    case CMD_CONNPATH_REQUEST:
      if (len > 1 && from.sourceProtocol == MeshAddress::SRC_SERIAL) {
        mConnectedPath->receiveUartPacket(data + 1, len - 1);
        err = 0;
      }
      break;
  ...
  if (err == HANDLE_UART_ERROR && mFromAddress.sourceProtocol != MeshAddress::SRC_BROADCAST) {
    ...
    LIB_LOGE(TAG, "Impl::handleFrame error frame %02X %02X size %d", data[0], data[1], len);
    delete[] rep;
  }
}

void EspMeshMesh::Impl::replyHandleFrame(const uint8_t *buf, uint16_t len, const MeshAddress &from, int16_t rssi) {
...
void EspMeshMesh::Impl::sendLog(int level, const char *tag, const char *payload, size_t payload_len) {
