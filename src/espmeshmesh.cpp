...
void EspMeshMesh::user_broadcast_recv_cb(uint8_t *data, uint16_t size, uint8_t *from) {
  if (singleton) {
    singleton->user_broadcast_recv(data, size, from);
  }
}

void EspMeshMesh::user_broadcast_recv(uint8_t *data, uint16_t size, uint8_t *from) {
